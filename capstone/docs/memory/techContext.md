...
make build-cpu       # build all app images (capstone-*:cpu tags)
make up-cpu          # start app stack with CPU detector images

# GPU workflow (NVIDIA)
make build-gpu       # build all app images (capstone-*:cuda tags)
...
```bash
# Produces: capstone-car-detector:cuda, capstone-person-detector:cuda
